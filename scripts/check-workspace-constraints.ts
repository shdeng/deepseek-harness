...
/** Private workspace assemblies used only as package-manager deploy roots. */
const internalWorkspacePackages = new Set(['@deepseek-ai/dsh-desktop-runtime'])
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    && (manifest.name === undefined || !internalWorkspacePackages.has(manifest.name))) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@deepseek-ai/')
    && !internalWorkspacePackages.has(manifest.name)) {
