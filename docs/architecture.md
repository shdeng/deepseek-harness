...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-gui-app`](../packages/bundle/gui-app/README.md) adds the transport-neutral graphical Host and client composition. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds HTTP, WebSocket, and static-asset carriage; [`dsh-desktop-app`](../packages/bundle/desktop-app/README.md) selects the no-listener Tauri composition. [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.

The Desktop bundle keeps Node as the policy and composition authority while mounting Rust implementations through `ctx.desktopNative`. Directory selection, controlled external links, notifications, application metadata, isolated companion windows, and deep links cross the private Host protocol; system-vault credentials cross WebView and stdio only as opaque references. Filesystem policy and subprocess streaming remain in Node.
