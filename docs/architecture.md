...
The Desktop bundle keeps Node as the policy and composition authority while mounting Rust implementations through `ctx.desktopNative`. Directory selection, controlled external links, notifications, application metadata, and deep links cross the private Host protocol; system-vault credentials cross WebView and stdio only as opaque references. Filesystem policy and subprocess streaming remain in Node.
