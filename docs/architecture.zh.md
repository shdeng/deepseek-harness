...
Desktop 组合包继续让 Node 负责策略与组合，并通过 `ctx.desktopNative` 挂载 Rust 实现。目录选择、受控外链、通知、应用元数据、隔离伴随窗口和深链接通过 Host 私有协议；系统凭据库的凭据在 WebView 与 stdio 中只以不透明引用传递。文件系统策略与 subprocess 流式执行仍留在 Node。
