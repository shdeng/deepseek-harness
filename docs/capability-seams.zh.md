...
flowchart LR
  ...
  pkg_credentials_system["credentials-system"]
  ...
  pkg_credentials_system --> svc_credentials
  pkg_desktop_native --> svc_desktopNative
  pkg_directory_picker --> svc_directoryPicker
  pkg_directory_picker_browse --> svc_directoryPicker
  pkg_directory_picker_desktop --> svc_directoryPicker
...
| `ctx.directoryPicker` | `seam` | `directory-picker` | `directory-picker-native`, `directory-picker-browse`, `directory-picker-desktop` | `apiproxy` | - | 带判别标记的交互能力：原生后端在 Host 显示设备上打开一个操作系统选择器，浏览后端为应用内浏览器提供列表与创建原语；双端后端通过其浏览器侧填充 ui-workspace 目录流程的 slot（不通过协议发布）。 |
