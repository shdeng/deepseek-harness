...
私有 `DSH-IPC/1` 通道是双向的。Node 为目录选择、受控 HTTP(S) 打开、通知、应用元数据和安全凭据采集发送有界原生请求。Rust 把已接受的 `deepseek-harness://session/<session-id>` 链接作为 `desktopNative/deep-link` 发回。双方均校验各操作的字段并关联请求 id；任何操作都不接受任意 command 名称或待执行的本地路径。
...
主窗口 Tauri ACL 只包含事件监听和三个应用 RPC command，不向 WebView 授予 dialog、opener、notification、credential、metadata 或 deep-link 插件 command。因此外链 anchor 与任务完成通知先调用 Host Remote 方法，再通过 `ctx.desktopNative` 到达 Rust。
