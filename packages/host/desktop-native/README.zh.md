...
这是由受监督 Rust Desktop 壳层持有的操作系统操作 Service Definition。`ctx.desktopNative` 向 Host Consumer 提供目录选择、原生凭据采集、受控 HTTP(S) 外链打开、系统通知、应用包元数据、隔离的媒体伴随窗口，以及已接受的深链接事件。Node Provider 位于 Desktop CLI 启动过程，通过私有 `DSH-IPC/1` 反向请求通道工作；主 WebView 没有这些操作对应的 Tauri command。
