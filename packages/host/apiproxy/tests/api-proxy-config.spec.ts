...
describe('credentials domain', () => {
  ...
  it('describes value-free views and flips state through set/unset with frames', async () => {
    ...
    expect(before.credentials).toEqual({ OPENAI_API_KEY: { configured: false, writable: true, input: 'value' } })
    const frames = await collectHost(api, ['host/remote-event'], 2, async () => {
      ...
      expect(after.credentials).toEqual({ OPENAI_API_KEY: { configured: true, source: 'file', writable: true, input: 'value' } })
  ...
  it('maps a shadowed write onto credential-rejected for set and unset alike', async () => {
    ...
    expect(described.credentials['DEEPSEEK_API_KEY']).toEqual({ configured: true, source: 'env', writable: false, input: 'value' })
