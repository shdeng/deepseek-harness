...
The built-in cards cover the shell executor (`bash`), agent-loop parallelism (`agent-loop`), the exclusive Desktop companion (`companion`), and the DeepSeek search provider (`web-search-deepseek`).
