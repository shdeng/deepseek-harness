...
Node 侧会扫描已启用的 Loader 配置项以发现 WebView `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并提供传输无关的资源读取。`dsh-web-app` 把这些读取及 sourcemap 映射到 `/plugins`；Tauri 壳层把 release 读取映射到不透明 `dsh-plugin` custom-protocol URL。源码启动会把 Host 侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
