...
The Node half scans enabled Loader entries for WebView `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and exposes transport-neutral asset reads. `dsh-web-app` maps those reads and source maps to `/plugins`; the Tauri shell maps release reads to opaque `dsh-plugin` custom-protocol URLs. Source launch maps Host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
