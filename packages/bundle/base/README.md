...
The shared dsh core as a profile bundle: [`cordis.patch.yml`](cordis.patch.yml) inserts every base plugin row — the DeepSeek model adapter, the shared [`agent-default-model`](../../core/agent-default-model/README.md) selection, tools, persistence, policy, settings/credentials, telemetry, and host-level in-process subagent providers — over the empty profile root, as the first layer of every profile's `dsh.profile.bundles` list. Product subagent providers and the optional [`pi-ai` multi-provider adapter](../llm-multi-provider/README.md) live in separate packages, so a deployment does not carry their SDK closures unless its composition selects them. Later bundle layers (e.g. [`dsh-web-app`](../web-app/README.md)) and the user's profile `cordis.patch.yml` override these rows by id; a patch replaces a row's whole `config`, so mode-specific values live in mode bundles, not here. The package has no runtime API; the profile composer resolves the patch through the `dsh.bundle.patch` manifest field, never through code.
